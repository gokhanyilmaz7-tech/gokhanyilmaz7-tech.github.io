...
function exportWord() {
  const reportHtml = items().map((item, index) => `<h2>${index + 1}. ${esc(item.title || item.location)}</h2><p class="meta">${esc(item.sectionTitle)} · ${esc(item.location)}</p>${item.html || `<p>${esc(item.text)}</p>`}`).join('<hr>');
