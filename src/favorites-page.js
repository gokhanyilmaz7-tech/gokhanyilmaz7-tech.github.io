...
function renderStream() {
  ...
  stream.innerHTML = visible.length ? visible.map((item) => { const listItems = selectedList === 'all' ? allItems() : data.lists.find((list) => list.id === selectedList)?.items || []; const index = listItems.findIndex((entry) => entry.id === item.id); const moveButtons = `<div class="favorite-card-reorder" aria-label="Favori hükmü sırala"><button data-move="up" data-item="${item.id}" ${index <= 0 ? 'disabled' : ''} aria-label="Hükmü yukarı taşı" title="Yukarı taşı">↑</button><button data-move="down" data-item="${item.id}" ${index < 0 || index >= listItems.length - 1 ? 'disabled' : ''} aria-label="Hükmü aşağı taşı" title="Aşağı taşı">↓</button></div>`; return `<div class="favorite-provision-shell"><button class="favorite-card-position" data-position="${item.id}" type="button" aria-label="Sıra numarasını değiştir" title="Bu hükmü doğrudan başka sıraya taşı">${index + 1}</button><article class="favorite-provision-card"><div class="favorite-card-main"><div class="favorite-card-meta">${esc(item.sectionTitle)} · ${esc(item.location)}</div><h2>${esc(item.title || item.location)}</h2><div class="favorite-provision-rich-text">${item.html ? normalizeFavoriteHtml(item.html) : `<p>${esc(item.text)}</p>`}</div><a class="favorite-source-link" href="/mevzuat.html?id=${encodeURIComponent(item.sectionId)}">Seçili mevzuatta aç →</a></div><div class="favorite-card-actions"><button data-edit="${item.id}">Başlığı değiştir</button><button data-remove="${item.id}">Listeden çıkar</button>${moveButtons}</div></article></div>`; }).join('') : '<div class="favorite-empty"><span>☆</span><h2>Henüz favori hüküm yok</h2><p>Seçili mevzuat sayfasında mavi yıldızlara tıklayarak hüküm ekleyebilirsiniz.</p></div>';
