...
function renderTools() {
  ...
  tools.innerHTML = `<div class="side-tools-heading"><span>FAVORİ ARAÇLARI</span><strong>${allItems().length} hüküm</strong></div><div class="favorite-list-buttons"><button data-list="all" class="${selectedList === 'all' ? 'active' : ''}">☆ Tümü (${allItems().length})</button>${data.lists.map((list) => `<button data-list="${list.id}" class="${selectedList === list.id ? 'active' : ''}">▸ ${esc(list.name)} <small>${list.items.length}</small></button>`).join('')}</div><a class="side-tool-button report-link-button" href="/noksanlik-raporu.html">＋ Noksanlık Raporu</a><button id="new-favorite-list" class="side-tool-button">＋ Favori listesi oluştur</button><button id="rename-favorite-list" class="side-tool-button" ${selectedList === 'all' ? 'disabled' : ''}>✎ Liste başlığını değiştir</button><button id="sort-favorites" class="side-tool-button">↕ Sıralama: ${sortMode === 'manual' ? 'özel sıra' : sortMode === 'latest' ? 'yeniden eskiye' : sortMode === 'oldest' ? 'eskiden yeniye' : 'başlığa göre'}</button>`;
