...
function renderTools() {
  ...
  const transferAllBtn = tools.querySelector('#transfer-all-favorites');
...
function renderStream() {
  ...
  stream.innerHTML = visible.length ? visible.map((item) => { const listItems = selectedList === 'all' ? allItems() : data.lists.find((list) => list.id === selectedList)?.items || []; const index = listItems.findIndex((entry) => entry.id === item.id); const moveButtons = `<div class="favorite-card-reorder" aria-label="Favori hükmü sırala"><button data-move="up" data-item="${item.id}" ${index <= 0 ? 'disabled' : ''} aria-label="Hükmü yukarı taşı" title="Yukarı taşı">↑</button><button data-move="down" data-item="${item.id}" ${index < 0 || index >= listItems.length - 1 ? 'disabled' : ''} aria-label="Hükmü aşağı taşı" title="Aşağı taşı">↓</button></div>`; const reported = reportItems(data).some((entry) => (entry.sourceId || entry.id) === item.id); return `<div class="favorite-provision-shell"><button class="favorite-card-position" data-position="${item.id}" type="button" aria-label="Sıra numarasını değiştir" title="Bu hükmü doğrudan başka sıraya taşı">${index + 1}</button><article class="favorite-provision-card"><div class="favorite-card-main"><div class="favorite-card-meta">${esc(item.sectionTitle)}</div>${item.title ? `<h2>${esc(item.title)}</h2>` : ''}<div class="favorite-provision-rich-text">${item.html ? normalizeFavoriteHtml(item.html) : `<p>${esc(item.text)}</p>`}</div><a class="favorite-source-link" href="${sourceHref(item)}">Seçili mevzuatta aç →</a></div><div class="favorite-card-actions"><button data-edit="${item.id}">Başlığı değiştir</button><button data-remove="${item.id}">Listeden çıkar</button><button data-transfer="${item.id}" ${reported ? 'disabled' : ''}>${reported ? 'Tedbirlerde' : 'Tedbirlere aktar'}</button>${moveButtons}</div></article></div>`; }).join('') : '<div class="favorite-empty"><button type="button" id="open-legislation-modal" class="favorite-empty-star" aria-label="Mevzuat listesini aç" title="Mevzuat seçmek için tıklayın">☆</button><h2>Henüz favori hüküm yok</h2><p>Seçili mevzuat sayfasında mavi yıldızlara tıklayarak hüküm ekleyebilirsiniz.</p></div>';
